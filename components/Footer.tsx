        ...
        <div className="footer-layout" style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr', gap: 60, marginBottom: 56 }}>
          <div>
            ...
            <div style={{ marginTop: 22, display: 'flex', gap: 12 }}>
              <a href="https://www.instagram.com/alasi_awning/" target="_blank" rel="noopener noreferrer" aria-label="Alasi Instagram" style={{ width: 34, height: 34, border: '1px solid rgba(216,196,168,0.2)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', textDecoration: 'none' }}>
          ...
          <div>
            <p style={{ fontSize: 9, letterSpacing: '0.18em', textTransform: 'uppercase', color: 'var(--c-sand)', marginBottom: 18, opacity: 0.5 }}>Company</p>
            {['About Alasi','Projects','Installation','FAQ','Contact'].map(item => (
              <a key={item} href="#" style={{ display: 'block', fontSize: 12, color: 'var(--c-sand)', textDecoration: 'none', marginBottom: 9, opacity: 0.6 }}>{item}</a>
            ))}
          </div>
          <div>
            <p style={{ fontSize: 9, letterSpacing: '0.18em', textTransform: 'uppercase', color: 'var(--c-sand)', marginBottom: 18, opacity: 0.5 }}>Contact</p>
            {['+995 591 00 00 00','info@alasi.ge','21 Rustaveli Ave','Batumi 6000','Georgia'].map(item => (
              <a key={item} href="#" style={{ display: 'block', fontSize: 12, color: 'var(--c-sand)', textDecoration: 'none', marginBottom: 9, opacity: 0.6 }}>{item}</a>
            ))}
          </div>
        </div>
        <div style={{ borderTop: '1px solid rgba(216,196,168,0.08)', paddingTop: 24, display: 'flex', justifyContent: 'space-between' }}>
          <p style={{ fontSize: 11, color: 'var(--c-sand)', opacity: 0.35 }}>© 2026 Alasi. All rights reserved.</p>
          <p style={{ fontSize: 10, color: 'var(--c-sand)', opacity: 0.25, letterSpacing: '0.12em', textTransform: 'uppercase' }}>Premium Awning Systems · Georgia</p>
        </div>
      </div>
    </footer>
  );
